...
export default function EditProfile() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Fetch the latest data after update so the UI stays synced
      // with the backend.
      const updatedProfile = await profileService.getProfile();

      setFullName(updatedProfile.fullName ?? "Eleanor St. James");
      setEmail(updatedProfile.email ?? "eleanor.sj@eventhub.com");
      setPhoneNumber(updatedProfile.phoneNumber ?? "+1 (555) 012-3456");
      setCity(updatedProfile.city ?? "San Francisco, CA");
      setAvatarUrl(
        updatedProfile.avatarUrl || "https://i.pravatar.cc/160?img=47"
      );
    } catch (error) {
